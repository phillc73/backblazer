...
b2UpdateBucket <- function(bucketId, bucketType){
  ...
  # API call
  b2Return <- httr::POST(paste(accountAuthorization$apiUrl,"/b2api/v1/b2_update_bucket", sep=""), body = jsonlite::toJSON(unbox(vars), pretty = TRUE), add_headers('Authorization' = as.character(accountAuthorization$authorizationToken)))
